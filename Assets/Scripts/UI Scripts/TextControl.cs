...
public class TextControl : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("CarCollider") || collision.CompareTag("textTrigger(5)"))
        {
            if(EnemyIsDead == true)
            {
                carsVoice.text = " Looks like the enemy dropped an auto-repair kit. Use it to restore health.";
            }
            
        }

        if (collision.CompareTag("CarCollider") || collision.CompareTag("textTrigger(6)"))
        {
